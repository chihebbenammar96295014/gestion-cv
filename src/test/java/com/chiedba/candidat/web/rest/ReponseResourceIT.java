...
class ReponseResourceIT {
    ...
    public static Reponse createEntity(EntityManager em) {
        Reponse reponse = new Reponse().order(DEFAULT_ORDER).content(DEFAULT_CONTENT);
        return reponse;
    }
    ...
    public static Reponse createUpdatedEntity(EntityManager em) {
        Reponse reponse = new Reponse().order(UPDATED_ORDER).content(UPDATED_CONTENT);
        return reponse;
    }
    ...
    void createReponse() throws Exception {
        ...
        assertThat(testReponse.getOrder()).isEqualTo(DEFAULT_ORDER);
        assertThat(testReponse.getContent()).isEqualTo(DEFAULT_CONTENT);
    }
    ...
    void putExistingReponse() throws Exception {
        ...
        updatedReponse.order(UPDATED_ORDER).content(UPDATED_CONTENT);
        ReponseDTO reponseDTO = reponseMapper.toDto(updatedReponse);

        restReponseMockMvc
            .perform(
                put(ENTITY_API_URL_ID, reponseDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(reponseDTO))
            )
        ...
        Reponse testReponse = reponseList.get(reponseList.size() - 1);
        assertThat(testReponse.getOrder()).isEqualTo(UPDATED_ORDER);
        assertThat(testReponse.getContent()).isEqualTo(UPDATED_CONTENT);
    }
    ...
    void partialUpdateReponseWithPatch() throws Exception {
        ...
        partialUpdatedReponse.order(UPDATED_ORDER).content(UPDATED_CONTENT);
    ...
    void deleteReponse() throws Exception {
        ...
        // Delete the reponse
        restReponseMockMvc
            .perform(delete(ENTITY_API_URL_ID, reponse.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
