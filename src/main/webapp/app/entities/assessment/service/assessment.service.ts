...
type RestOf<T extends IAssessment | NewAssessment> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class AssessmentService {
  ...
  protected convertDateFromClient<T extends IAssessment | NewAssessment | PartialUpdateAssessment>(assessment: T): RestOf<T> {
    return {
      ...assessment,
      date: assessment.date?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAssessment: RestAssessment): IAssessment {
    return {
      ...restAssessment,
      date: restAssessment.date ? dayjs(restAssessment.date) : undefined,
    };
  }
